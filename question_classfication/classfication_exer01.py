...
class Question_classify():
    ...
    def read_train_data(self):
        train_x=[]
        train_y=[]
        file_list=getfilelist("../question_classfication/data_question/")
        # file_list = getfilelist("./data_question/")
        # 遍历所有文件
...
if __name__ == '__main__':
    ...
    test_sentence = "灵隐寺是什么类型"
    print(qc.predict(test_sentence))
    list = pyhanlp.HanLP.segment(test_sentence)
    print(list)
    word = []
    flag = []
    for item in list:
        s = str(item)
        word.append(s.split('/')[0])
        flag.append(s.split('/')[1])
    print(word)
    print(flag)