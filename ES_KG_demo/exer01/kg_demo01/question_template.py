...
class QuestionTemplate():
    ...
    def get_address(self):
        spot_name = self.get_spot_name()
        cql = f"match(p:Place)-[r:locatedin]->(b) where p.title='{spot_name}' return b.areport"
        print(cql)
        answer = self.graph.run(cql)[0]
    ...
    def get_introduction(self):
        spot_name = self.get_spot_name()
        cql = f"match(p:Place)-[]->() where p.title='{spot_name}' return p.abstract"
        print(cql)
        answer = self.graph.run(cql)[0]
    ...
    def get_opentime(self):
        spot_name = self.get_spot_name()
        cql = f"match(p:Place)-[]->() where p.title='{spot_name}' return p.opening_hours"
        print(cql)
        answer = self.graph.run(cql)[0]
